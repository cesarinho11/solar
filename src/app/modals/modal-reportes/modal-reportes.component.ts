  ...
  generarReporte() {
    if (this.data.tipo == 1) {
      ...
      if (this.formato == 'pdf') {
        ...
        this.reportService.totalVentasSinAgrupar(this.reporteForm.value).subscribe({
          next: (res: any) => {
            console.log('sin agrupar', res);
...
console.log('entro', this.reporteForm.value);
  ...
  //REPORTE DE VENTAS
  reportExcelVentas(nameReport: string, data: any[], fechaInicio: string, fechaFin: string, total: any) {
    console.log('entro');
